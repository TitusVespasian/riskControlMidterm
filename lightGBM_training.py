...
best_params={}
best_params['n_estimators']=len(cv_results['auc-mean'])
...
gsearch1 = GridSearchCV(
    estimator=lgb.LGBMClassifier(boosting_type='gbdt', objective='binary', metrics='auc', learning_rate=0.1,
                                 n_estimators=best_params['n_estimators'], max_depth=6, bagging_fraction=0.8, feature_fraction=0.8,
                                 is_unbalance=True),
    param_grid=params_test1, scoring='roc_auc', cv=5, n_jobs=-1)
gsearch1.fit(X_train, y_train)
print(gsearch1.cv_results_, gsearch1.best_params_, gsearch1.best_score_)
# {'max_depth': 5, 'num_leaves': 10} 0.7334410002865965
best_params=best_params.items()+gsearch1.best_params_.items()
