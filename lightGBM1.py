...
def temp():
    ...
    X["UserInfo_22"]=X["UserInfo_22"].apply(lambda x: x if x=="D" or x=="未婚" or x=="已婚" or x=="不详" else "未婚")
    #X.loc[X['UserInfo_22'] == '初婚', 'UserInfo_22'] = "已婚"
    X['UserInfo_23'] = X['UserInfo_23'].apply(lambda x: x if x=="大学本科（简称“大学" or x=="H" or x=="M" or x=="D" or x=="AC" or x=="Y"
                                              else "rest")
    X['UserInfo_24'] = X['UserInfo_24'].apply(lambda x: 1 if x == 'D' else 0)
    X['Education_Info7'] = X['Education_Info7'].apply(lambda x: 1 if x == 'E' else 0)
    X['Education_Info2'] = X['Education_Info2'].apply(lambda x: 1 if x == 'E' else 0)
    X['Education_Info8']=X['Education_Info8'].apply(lambda x:x if x=="E" or x=="T" or x=="F" or x=="不详" else "rest")
    X['WeblogInfo_19']=X['WeblogInfo_19'].apply(lambda x:x if x=="D" or x=="不详" or x=="I" or x=="G" else "rest")
    X['WeblogInfo_20']=X['WeblogInfo_20'].apply(lambda x:x if x=="I4" or x=="I3" or x=="C1" or x=="C21" or x=="I5" or x=="不详" else "rest")
    ...
    train_all_y=train_all["target"]
    train_all=train_all.drop(["Idx0", "target"], axis=1)
    ...
    X_resampled, y_resampled = SMOTE().fit_resample(train_all,train_all_y)
    ...
    test_all = test_all.drop(["Idx0", "target"], axis=1)
    ...
    test_res = clf.predict_proba(test_all)[:, 1]
